...
Load observed fire event data.
```{r}
observed.df <- read.csv(paste0(dataDir,'/manual_fires_observed.csv'))
...
Execute the method for building a composite biomass burning record used in Kelly et al. 2013 (10.1073/pnas.1305069110). The method estimates the parameters of a zero-inflated log-normal (ZIL) distribution of pooled charcoal counts in continuous moving windows of a user-defined width. In this analysis, 5-year and 50-year window widths are used (2.5 and 25 half-kernel widths, respectively), so the procedure runs twice, onces for each window width. 

This portion of the analysis is relegated to a separate R script for clarity of the workflow. The user-defined parameters, however, can be adjusted here.
```{r}
# Interval for fitted x's
timeStep <- 
gih
```
