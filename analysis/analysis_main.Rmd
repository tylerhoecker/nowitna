...
combined.1900_2010.df <- composite.df %>%
  ...
  left_join(., tree.cor.df, by = 'ageCE') %>%
  select(lowMean, highMean, win.pct, cru.precip = precip, cru.temp = temp, goa.temp = bin.temp, all)



```

Calculate correlation among fire, tree, and climate records.
```{r}
corr.1550_1895.df <- rcorr(as.matrix(combined.1550_1895.df), type="spearman") 
corr.1900_2010.df <- rcorr(as.matrix(combined.1900_2010.df), type="spearman") 
...
ggplot(combined.1900_2010.df, aes(x = lowMean, y = cru.temp)) +
  ...
  theme_bw(base_size = 12) +
  labs(x = '50-yr mean biomass burning', y = 'Modeled summer temp.',title = '1900-2015')
```
